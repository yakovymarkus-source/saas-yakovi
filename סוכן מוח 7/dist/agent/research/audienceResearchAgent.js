...
exports.runAudienceResearch = runAudienceResearch;
function buildPersonaLabel(business) {
    if (business.audienceHint?.trim())
        return business.audienceHint.trim();
    if (business.goals.primary === 'appointments')
        return 'ליד חם שמפחד לבזבז זמן על שיחה לא מדויקת';
    if (business.goals.primary === 'sales')
        return 'קונה ספקן שמחפש ודאות לפני רכישה';
    if (business.goals.primary === 'leads')
        return 'לקוח עם כאב ברור אבל עם אמון שבור';
    return 'קהל קר שצריך להבין למה זה רלוונטי אליו עכשיו';
}
async function runAudienceResearch(business, market) {
