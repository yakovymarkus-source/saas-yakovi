import { CampaignBuildOutput } from '../domain/campaignBuild';
import {
  AdCreativePack,
  AssetIterationMeta,
  AssetQualityScore,
...
import { PerformanceDiagnosis } from '../domain/optimization';
...
function containsHyperbolicClaim(text: string): boolean {
...
function deriveStatus(total: number): AssetQualityScore['status'] {
  if (total < IMPROVE_THRESHOLD) return 'reject';
  if (total < PASS_THRESHOLD) return 'improve';
  return 'pass';
}

export function scoreAssetText(text: string, angle: string, persona: string, cta: string): AssetQualityScore {
  const reasons: string[] = [];
  const blockedBy: string[] = [];
  const normalizedText = text.replace(/\s+/g, ' ').trim();
  const concreteSignals = countConcreteSignals(normalizedText);

  const clarity = normalizeScore(
    52 +
  ...
  const emotionalImpact = normalizeScore(
    45 +
  ...
  const conversionStrength = normalizeScore(
    40 +
  ...
  const strategyAlignment = normalizeScore(
    42 +
  ...
  if (clarity < IMPROVE_THRESHOLD) reasons.push('clarity too weak');
  if (specificity < IMPROVE_THRESHOLD) reasons.push('specificity too weak');
  if (emotionalImpact < IMPROVE_THRESHOLD) reasons.push('emotional impact too weak');
  if (conversionStrength < IMPROVE_THRESHOLD) reasons.push('conversion strength too weak');
  if (strategyAlignment < IMPROVE_THRESHOLD) reasons.push('strategy alignment too weak');
...
function buildIteration(assetType: CampaignAssetType, attempt: number, score: AssetQualityScore): AssetIterationMeta {
  return {
    assetType,
    attempt,
...
function qualityGuard(text: string, label: string) {
  if (containsHyperbolicClaim(text)) {
    throw new HttpError(400, `Campaign rules rejected ${label}`, { reason: 'Asset contains exaggerated promise' });
  }
  if (containsAny(text, genericPhrases)) {
    throw new HttpError(400, `Campaign rules rejected ${label}`, { reason: 'Asset contains generic marketing language' });
  }
}

export function validateStrategyConsistency(strategy: CampaignBuildOutput): void {
  ...
  if (!finalStep) {
    throw new HttpError(400, 'Campaign rules rejected strategy', { reason: 'Funnel is missing final conversion step' });
  }

  const ctaMatches =
    (strategy.offer.ctaType === 'book_call' && /שיחה|call/i.test(finalStep.cta)) ||
  ...
  if (!ctaMatches) {
    throw new HttpError(400, 'Campaign rules rejected strategy', { reason: 'Offer CTA and funnel closing CTA are not aligned' });
  }

  if (!strategy.offer.offerStructure.some((item) => item.includes(strategy.positioning.coreAngle))) {
    throw new HttpError(400, 'Campaign rules rejected strategy', { reason: 'Offer structure drifted away from core angle' });
  }
  ...
  if (!strategy.funnel.steps.some((step) => step.message.includes(strategy.positioning.uniqueMechanism) || step.message.includes(strategy.business.targetOutcome))) {
    throw new HttpError(400, 'Campaign rules rejected strategy', { reason: 'Funnel lost the core mechanism or target outcome' });
  }
}

function ctaLabel(ctaType: CampaignBuildOutput['offer']['ctaType']): string {
  if (ctaType === 'buy_now') return 'לקנייה עכשיו';
  if (ctaType === 'book_call') return 'לקביעת שיחה';
  if (ctaType === 'apply_now') return 'להגשת מועמדות';
  return 'להשארת פרטים';
}

function rankVariants<T extends { qualityScore?: AssetQualityScore; selected?: boolean }>(
  items: T[],
  getId: (item: T, index: number) => string
): RankedVariant[] {
  const ranked = items
    .map((item, index) => ({ item, index, id: getId(item, index), total: item.qualityScore?.total ?? 0, strategicFit: item.qualityScore?.strategyAlignment ?? 0, status: item.qualityScore?.status ?? 'reject' as const }))
...
export function qualifyLandingCopy(strategy: CampaignBuildOutput, landingCopy: LandingPageCopy, attempt = 1): LandingPageCopy {
...
export function qualifyAdPack(strategy: CampaignBuildOutput, adPack: AdCreativePack, attempt = 1): AdCreativePack {
  const angle = strategy.positioning.coreAngle;
  const persona = strategy.audience.corePersona.label;
  ...
  adPack.ads.forEach((ad, index) => {
    ...
    if (hooks.has(ad.hook)) {
      throw new HttpError(400, 'Campaign rules rejected ad pack', { reason: 'Ad variations are too similar' });
    }
    hooks.add(ad.hook);

    if (!ad.angle.toLowerCase().includes(angle.split(' ')[0]?.toLowerCase() ?? '')) {
      throw new HttpError(400, 'Campaign rules rejected ad pack', { reason: 'Ad angle drifted away from selected positioning angle' });
    }

    if (ad.cta !== requiredCta && !ad.cta.toLowerCase().includes(strategy.offer.ctaType.split('_')[0])) {
      throw new HttpError(400, 'Campaign rules rejected ad pack', { reason: 'Ad CTA contradicts selected offer CTA' });
    }
...
export function qualifyVideoPack(strategy: CampaignBuildOutput, videoPack: VideoScriptPack, attempt = 1): VideoScriptPack {
  const angle = strategy.positioning.coreAngle;
  const persona = strategy.audience.corePersona.label;
  const requiredCta = ctaLabel(strategy.offer.ctaType);
  ...
  videoPack.scripts.forEach((script, index) => {
    ...
    if (script.cta !== requiredCta && !script.cta.includes(requiredCta.split(' ')[0])) {
      throw new HttpError(400, 'Campaign rules rejected video script', { reason: 'Video CTA contradicts campaign CTA' });
    }
...
export function validateGeneratedAssets(input: {
  strategy: CampaignBuildOutput;
  landingCopy?: LandingPageCopy;
  adPack?: AdCreativePack;
  videoPack?: VideoScriptPack;
}): { landingCopy?: LandingPageCopy; adPack?: AdCreativePack; videoPack?: VideoScriptPack } {
...
export function validateAssetConsistency(
  strategy: CampaignBuildOutput,
  assets: { landingCopy?: LandingPageCopy; adPack?: AdCreativePack; videoPack?: VideoScriptPack }
): void {
  const angle = strategy.positioning.coreAngle;
  const cta = ctaLabel(strategy.offer.ctaType);
  ...
  if (assets.landingCopy) {
    ...
    if (!copyText.includes(cta)) {
      throw new HttpError(400, 'Campaign rules rejected landing page copy', { reason: 'Landing CTA contradicts the selected offer CTA' });
    }
    if (!copyText.toLowerCase().includes(angle.split(' ')[0]?.toLowerCase() ?? '')) {
      throw new HttpError(400, 'Campaign rules rejected landing page copy', { reason: 'Landing copy lost the campaign angle' });
    }
  }
  ...
  if (assets.adPack) {
    ...
    if (angleDrift || ctaDrift) {
      throw new HttpError(400, 'Campaign rules rejected ad pack', { reason: 'Ad pack contradicts campaign strategy' });
    }
  }
  ...
  if (assets.videoPack) {
    ...
    if (toneDrift) {
      throw new HttpError(400, 'Campaign rules rejected video script', { reason: 'Video scripts drifted away from the core promise/mechanism' });
    }
  }
}

export function assertProductionReadyCampaign(input: {
  strategy: CampaignBuildOutput;
  landingCopy: LandingPageCopy;
  adPack: AdCreativePack;
  videoPack: VideoScriptPack;
  ...
  if ((input.landingCopy.qualityScore?.status ?? 'reject') !== 'pass') failures.push('landing page did not pass quality gate');
  if (input.adPack.ads.some((ad) => (ad.qualityScore?.status ?? 'reject') !== 'pass')) failures.push('one or more ads did not pass quality gate');
  if (input.videoPack.scripts.some((script) => (script.qualityScore?.status ?? 'reject') !== 'pass')) failures.push('one or more video scripts did not pass quality gate');
  if (!input.adPack.selectedVariantId) failures.push('ad ranking missing selected launch asset');
  if (!input.videoPack.selectedVariantId) failures.push('video ranking missing selected launch asset');

  if (failures.length > 0) {
    throw new HttpError(400, 'Campaign build rejected', { reason: failures.join('; ') });
  }
}

export function improveLandingCopy(copy: LandingPageCopy, strategy: CampaignBuildOutput, reasons: string[], attempt: number): LandingPageCopy {
  const persona = strategy.audience.corePersona.label;
...
export function improveAdPack(adPack: AdCreativePack, strategy: CampaignBuildOutput, reasons: string[], attempt: number): AdCreativePack {
  const cta = ctaLabel(strategy.offer.ctaType);
  const persona = strategy.audience.corePersona.label;
...
export function improveVideoPack(videoPack: VideoScriptPack, strategy: CampaignBuildOutput, reasons: string[], attempt: number): VideoScriptPack {
  const cta = ctaLabel(strategy.offer.ctaType);
  const persona = strategy.audience.corePersona.label;
...
export function improveAssetFromDiagnosis(assetType: CampaignAssetType, content: any, diagnosis: PerformanceDiagnosis): any {
  ...
  if (assetType === 'landing_page' && content?.copy) {
    return {
      ...
      copy: {
        ...
        bodySections: content.copy.bodySections.map((section: any, index: number) => ({
          ...section,
  ...
  if (assetType === 'ad' && content?.ads) {
    return {
      ...content,
      ads: content.ads.map((ad: any, index: number) => ({
        ...ad,
  ...
  if (assetType === 'video_script' && content?.scripts) {
    return {
      ...content,
      scripts: content.scripts.map((script: any, index: number) => ({
        ...script,
