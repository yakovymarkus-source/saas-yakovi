import { AudienceResearch, BusinessProfile, MarketResearch, OfferStrategy, PositioningDecision } from '../../domain/campaignBuild';

export async function runOfferStrategy(
  ...
  const trustLevel: OfferStrategy['trustLevel'] = audience.awarenessLevel === 'product_aware' || audience.awarenessLevel === 'most_aware'
    ? 'high'
  ...
  const offerType: OfferStrategy['offerType'] =
    business.goals.primary === 'leads'
      ? 'lead_magnet'
      : callDriven
        ? 'call_booking'
  ...
  const ctaType: OfferStrategy['ctaType'] =
    offerType === 'call_booking'
      ? 'book_call'
  ...
  if (premium && ctaType === 'buy_now') weakPoints.push('מחיר גבוה מדי לרכישה קרה ישירה.');
  if (trustLevel === 'low' && !valueStack.some((item) => /הוכחה|proof/i.test(item))) weakPoints.push('אין מספיק הוכחה בשלב מוקדם.');
  if (audience.awarenessLevel === 'problem_aware' && ctaType === 'buy_now') weakPoints.push('הקהל עדיין לא בשל לרכישה ישירה.');
