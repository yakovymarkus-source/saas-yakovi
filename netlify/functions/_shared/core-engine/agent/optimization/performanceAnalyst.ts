import { DiagnosisItem, PerformanceDiagnosis, PerformanceInput, RegenerationBrief } from '../../domain/optimization';
import { explainLikeHuman } from '../../engine/explainLikeHuman';

function pushIssue(issues: DiagnosisItem[], issue: DiagnosisItem) {
  issues.push(issue);
}

function addBrief(briefs: RegenerationBrief[], brief: RegenerationBrief) {
  briefs.push(brief);
}

export async function runPerformanceAnalysis(input: PerformanceInput): Promise<PerformanceDiagnosis> {
  const issues: DiagnosisItem[] = [];
  const briefs: RegenerationBrief[] = [];
  ...
  return {
    ...
    plainHebrew: explainLikeHuman(diagnosis)
  };
}
