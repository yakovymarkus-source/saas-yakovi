...
import { StrategyVerdict } from '../domain/strategy';

function scoreConfidence(strategy: CampaignBuildOutput): number {
  let score = 60;
  if ((strategy.audience.corePersona.deepPains ?? []).length >= 2) score += 8;
  if ((strategy.market.whitespaceOpportunities ?? []).length > 0) score += 7;
  if ((strategy.offer.weakPoints ?? []).length === 0) score += 10;
  if (strategy.offer.ctaType === 'book_call' && (strategy.business.pricing.amount ?? 0) >= 1000) score += 8;
  if (strategy.offer.ctaType === 'buy_now' && (strategy.business.pricing.amount ?? 0) <= 500) score += 8;
  if (strategy.audience.awarenessLevel === 'product_aware' || strategy.audience.awarenessLevel === 'most_aware') score += 5;
  if (strategy.market.marketStage === 'red_ocean') score -= 8;
  if ((strategy.business.constraints ?? []).length >= 3) score -= 5;
  return Math.max(0, Math.min(100, score));
}

export function buildStrategyVerdict(strategy: CampaignBuildOutput): StrategyVerdict {
