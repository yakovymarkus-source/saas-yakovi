...
const { TOTAL_STEPS }  = require('./_shared/analysis/pipeline');
...
exports.handler = async (event) => {
  ...
  const { data: job, error: jobErr } = await db.from('analysis_jobs')
    .select('id, status, overall_score, report_id, error_message, ai_calls_used, generation_ms')
    .eq('id', jobId).eq('user_id', user.id).single();

  if (jobErr || !job) return { statusCode: 404, body: JSON.stringify({ error: 'Job not found' }) };

  const { data: steps } = await db.from('analysis_steps')
