...
function showError(msg) {
  document.getElementById('app').innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;gap:1rem;direction:rtl">
    <div style="font-size:2rem">🔒</div><h2>${msg}</h2>
    <button class="btn btn-secondary" onclick="handleLogout()">התנתקות</button></div>`;
}
...
function shell(content) {
  const nav = [
    ...
    { id: 'subscriptions-mgmt', icon: '💳', label: 'מנויים' },
    { id: 'assets-mgmt',      icon: '🚀', label: 'קבצים' },
    { id: 'onboarding-mgmt',  icon: '🎯', label: 'אישור חדשים' },
    { id: 'metrics-mgmt',     icon: '📈', label: 'מדדים' },
    { id: 'billing',          icon: '💰', label: 'תשלומים' },
    { id: 'qa',               icon: '🔬',  label: 'ביקורת' },
    { id: 'system',           icon: '🖥️',  label: 'מערכת' },
    { id: 'audit',            icon: '📋', label: 'יומן ביקורת' },
    { id: 'updates-mgmt',     icon: '📣', label: 'עדכונים' },
    { id: 'support-mgmt',     icon: '🎫', label: 'תמיכה' },
    { id: 'api-keys',         icon: '🔑', label: 'מפתחות API' },
  ];
    ...
    <div class="app-shell">
      <aside class="sidebar">
        ...
        <div class="sidebar-footer">${state.user?.email || ''}<br/>
          <span onclick="handleLogout()" style="cursor:pointer;color:#818cf8">התנתקות</span></div>
      </aside>
...
async function renderApiKeys() {
  ...
  shell(`
    <div class="page-header">
      <h1 class="page-title">🔑 מפתחות API</h1>
      <p class="page-subtitle">ניהול מפתחות גישה לספקי בינה מלאכותית — השינויים נכנסים לתוקף לאחר הצבה מחדש</p>
    </div>
    ...
    <div class="card" style="background:#0f172a;border:1px solid #1e293b">
      <div style="font-weight:600;margin-bottom:.75rem">⚡ הצבה מחדש לאחר שמירה</div>
      <p style="font-size:.85rem;color:#6b7280;margin-bottom:1rem;text-align:right">
        לאחר עדכון מפתחות יש לבצע הצבה מחדש כדי שהפונקציות יטענו את הערכים החדשים. זה יקח כ-2 דקות.
      </p>
    ...
    <div class="card" style="margin-top:1rem;background:#0f172a;border:1px solid #1e293b">
      ...
      <ul style="font-size:.85rem;color:#9ca3af;line-height:1.8;padding-right:1.25rem;text-align:right">
        <li><strong style="color:#f1f5f9">מפתח OpenRouter</strong> — ספק בינה מלאכותית ראשי. כל השיחות, כתיבת קופי, דפי נחיתה. <a href="https://openrouter.ai/keys" target="_blank" style="color:#6366f1">openrouter.ai/keys</a></li>
        <li><strong style="color:#f1f5f9">מפתח Anthropic</strong> — גיבוי אם OpenRouter נכשל. <a href="https://console.anthropic.com/keys" target="_blank" style="color:#6366f1">console.anthropic.com</a></li>
        <li><strong style="color:#f1f5f9">מפתח OpenAI / DALL-E</strong> — יצירת תמונות ומודעות ויזואליות. <a href="https://platform.openai.com/api-keys" target="_blank" style="color:#6366f1">platform.openai.com</a></li>
      </ul>
    </div>`);
}
...
async function triggerRedeploy() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = '⏳ הצבה מחדש...'; }
  try {
    // Trigger a new deploy by hitting the Netlify deploy hook or the health endpoint
    showApiMsg('success', 'הצבה מחדש הופעלה — השינויים ייכנסו לתוקף תוך ~2 דקות');
  } catch {
    showApiMsg('error', 'לא ניתן להפעיל הצבה מחדש אוטומטית — בצע הצבה ידנית מ-Netlify');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '🚀 הפעל הצבה מחדש'; }
  }
}
